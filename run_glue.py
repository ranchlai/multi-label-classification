...
def main():
    ...
    if data_args.task_name is not None:
        ...
        if is_regression:
            ...
            if isinstance(raw_datasets["train"]["label"][0], list):
                is_multi_label = True
                logger.info("Label type is list, doing multi-label classification")

            # trying to find the number of labels in a multi-label classification task
            # We have to deal with cases that labels appear in the training set but not in the validation/test set
            # So we build the label list from the union of labels in train/val/test
            label_list = get_label_list(raw_datasets, split="train")
            for split in ["validation", "test"]:
                if split in raw_datasets:
                    test_labels = get_label_list(raw_datasets, split=split)
                    diff = set(test_labels).difference(set(label_list))
                    if len(diff) > 0:
                        logger.warning(
                ...
                with open(output_predict_file, "w") as writer:
                    ...
                    for index, item in enumerate(predictions):
                        if is_regression:
                            ...
                            item = [
                                label_list[i] for i in range(len(item)) if item[i] == 1
                            ]
